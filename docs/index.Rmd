...
```{r}
#total antidepressant prescriptions in that DataZone, regardless of which specific BNFItemCode
Prescription_byDataZone <- GPPracticeLocation_CombinedPrescriber_SIMD_Population %>%
  group_by(DataZone) %>%
  summarise(
    TotalPrescriptions = sum(BNFItemCode),
    TotalPatients = n_distinct(GPPractice),
    PrescriptionRate = (TotalPrescriptions / TotalPopulation) * 1000,
    .groups = "drop"
  )
...
summary(TotalPrescriptions_byGP2$PrescriptionRate_byGP)
summary(TotalPrescriptions_byGP2$PracticeListSize)
...
max(TotalPrescriptions_byGP$PrescriptionRate_byGP, na.rm = TRUE)
